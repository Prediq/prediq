...
if   [ ${rails_env} == 'development' ]; then
    cd ${current_path} && RAILS_ENV=${rails_env} bundle exec ruby app/classes/import_data.rb $func $rails_env $current_path $user_id
else
#    if [ ${rails_env} == 'import_staging' ]; then
    if [ ${rails_env} == 'staging' ]; then
        cd ${current_path} && RAILS_ENV=${rails_env} bundle exec ruby app/classes/import_data.rb $func $rails_env $current_path $user_id > "${current_path}/log/import_data_${rails_env}_${log_date}.log" 2>&1 &
    fi
...
# local:
# func              = ARGV[0] 'get_first_yr', 'get_five_yr' or 'get_daily'
# rails_env         = ARGV[1] 'development', 'staging', 'production'
# current_path      = ARGV[2]
...
# Local:
# current_path=/Users/billkiskin/prediq/prediq_api;func=get_first_yr;rails_env=development;api_customer_id=11; cd ${current_path} && RAILS_ENV=import_development app/classes/run_import_data.sh ${func} ${rails_env} ${current_path} ${api_customer_id}
# Staging (out of the 'prediq_api' rails stack):
# current_path=/var/www/vhosts/prediq_api;func=get_first_yr;rails_env=staging;api_customer_id=2; cd ${current_path} && RAILS_ENV=import_development app/classes/run_import_data.sh ${func} ${rails_env} ${current_path} ${api_customer_id}

# current_path=/Users/billkiskin/hiplogiq/pg_data;func=get_first_yr;rails_env=development; cd ${current_path} && app/classes/run_load_pg_data.sh ${func} ${rails_env} ${current_path}
